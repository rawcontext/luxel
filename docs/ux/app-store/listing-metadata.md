...
This file is the checked-in source of truth for Luxel's Mac App Store product-page metadata. It records the verified public listing as of August 22, 2026, the localized text metadata for version 1.3.0, and the localized screenshot gallery derived from the six English App Store masters in `docs/design`. Use the guarded App Store workflows to synchronize the editable App Store Connect version; do not edit the draft independently.
...
- [Run 32782940879](https://github.com/ccheney/luxel/actions/runs/32782940879) built, signed, and uploaded macOS version 1.3.0, build `202608242206`, to App Store Connect and TestFlight from release tag `v1.3.0`.
- [Run 32783943577](https://github.com/ccheney/luxel/actions/runs/32783943577) synchronized the concise localized Copy as Markdown What's New text and App Review notes for version 1.3.0.
- [Run 32784038369](https://github.com/ccheney/luxel/actions/runs/32784038369) selected build `202608242206`, submitted version 1.3.0 for App Review, and enabled automatic release after approval without a phased rollout.
...
```json
{
  ...
  "whatsNew": "You can now copy transcripts as Markdown.",
  ...
  "counts": { "name": 5, "subtitle": 24, "promotionalText": 134, "description": 2338, "whatsNew": 41, "keywordBytes": 88 }
}
```
...
```json
{
  ...
  "whatsNew": "Transkripte können jetzt als Markdown kopiert werden.",
  ...
  "counts": { "name": 5, "subtitle": 27, "promotionalText": 136, "description": 2476, "whatsNew": 53, "keywordBytes": 74 }
}
```
...
```json
{
  ...
  "whatsNew": "Ahora puedes copiar las transcripciones como Markdown.",
  ...
  "counts": { "name": 5, "subtitle": 30, "promotionalText": 121, "description": 2390, "whatsNew": 54, "keywordBytes": 77 }
}
```
...
```json
{
  ...
  "whatsNew": "Vous pouvez désormais copier les transcriptions au format Markdown.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 132, "description": 2539, "whatsNew": 67, "keywordBytes": 78 }
}
```
...
```json
{
  ...
  "whatsNew": "Ora puoi copiare le trascrizioni in formato Markdown.",
  ...
  "counts": { "name": 5, "subtitle": 30, "promotionalText": 128, "description": 2477, "whatsNew": 53, "keywordBytes": 72 }
}
```
...
```json
{
  ...
  "whatsNew": "文字起こしをMarkdown形式でコピーできるようになりました。",
  ...
  "counts": { "name": 5, "subtitle": 9, "promotionalText": 55, "description": 1097, "whatsNew": 32, "keywordBytes": 89 }
}
```
...
```json
{
  ...
  "whatsNew": "이제 대화 내용 기록을 Markdown 형식으로 복사할 수 있습니다.",
  ...
  "counts": { "name": 5, "subtitle": 12, "promotionalText": 66, "description": 1265, "whatsNew": 38, "keywordBytes": 92 }
}
```
...
```json
{
  ...
  "whatsNew": "Giờ đây, bạn có thể sao chép bản chép lời dưới dạng Markdown.",
  ...
  "counts": { "name": 5, "subtitle": 22, "promotionalText": 106, "description": 2146, "whatsNew": 61, "keywordBytes": 70 }
}
```
...
```json
{
  ...
  "whatsNew": "现在可以将转录文本复制为 Markdown 格式。",
  ...
  "counts": { "name": 5, "subtitle": 7, "promotionalText": 53, "description": 822, "whatsNew": 25, "keywordBytes": 86 }
}
```
...
```json
{
  ...
  "whatsNew": "Agora você pode copiar transcrições como Markdown.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 122, "description": 2280, "whatsNew": 50, "keywordBytes": 73 }
}
```
...
```json
{
  ...
  "whatsNew": "Agora pode copiar transcrições em formato Markdown.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 110, "description": 2352, "whatsNew": 51, "keywordBytes": 76 }
}
```

## Shared App Review notes

App Review notes are not a localized storefront field. Use the single English value in `docs/app-review/review-notes.txt`, which stays below Apple's 4,000-byte limit and explains how to review the version 1.3.0 Copy as Markdown feature.
