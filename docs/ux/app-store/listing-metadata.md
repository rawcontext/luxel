...
This file is the checked-in source of truth for Luxel's Mac App Store product-page metadata. It records the verified public listing as of August 22, 2026, the localized text metadata for version 1.2.2, and the localized screenshot gallery derived from the six English App Store masters in `docs/design`. Use the guarded App Store workflows to synchronize the editable App Store Connect version; do not edit the draft independently.
...
- [Run 32746887986](https://github.com/ccheney/luxel/actions/runs/32746887986) synchronized the localized What's New text and App Review notes for version 1.2.2 without changing the binary, screenshots, review submission, or release settings.
...
```json
{
  ...
  "whatsNew": "Speech Detection Prompts now require 10 seconds of detected speech before notifying you. When allowed, speech alerts use Time Sensitive notifications so in-the-moment prompts are less likely to be delayed by notification summaries or Focus. New controls show macOS notification access, and clicking an alert starts recording.",
  ...
  "counts": { "name": 5, "subtitle": 24, "promotionalText": 134, "description": 2338, "whatsNew": 325, "keywordBytes": 88 }
}
```
...
```json
{
  ...
  "whatsNew": "Spracherkennungs-Hinweise erfordern jetzt 10 Sekunden erkannter Sprache, bevor du benachrichtigt wirst. Wenn erlaubt, verwenden Sprachhinweise zeitkritische Mitteilungen, damit aktuelle Hinweise seltener durch Mitteilungszusammenfassungen oder Fokus verzögert werden. Neue Steuerelemente zeigen den macOS-Mitteilungszugriff, und ein Klick auf den Hinweis startet die Aufnahme.",
  ...
  "counts": { "name": 5, "subtitle": 27, "promotionalText": 136, "description": 2476, "whatsNew": 376, "keywordBytes": 74 }
}
```
...
```json
{
  ...
  "whatsNew": "Los avisos de detección de voz ahora requieren 10 segundos de voz detectada antes de avisarte. Si se permite, usan avisos importantes para que sea menos probable que los resúmenes de notificaciones o un modo de concentración retrasen un aviso oportuno. Los nuevos controles muestran el acceso a las notificaciones de macOS y, al hacer clic en un aviso, comienza la grabación.",
  ...
  "counts": { "name": 5, "subtitle": 30, "promotionalText": 121, "description": 2390, "whatsNew": 375, "keywordBytes": 77 }
}
```
...
```json
{
  ...
  "whatsNew": "Les alertes de détection vocale exigent désormais 10 secondes de parole détectée avant de vous prévenir. Si vous les autorisez, elles utilisent des notifications urgentes afin que les résumés de notifications ou un mode de concentration retardent moins les alertes immédiates. De nouvelles commandes indiquent l’accès aux notifications de macOS, et cliquer sur une alerte démarre l’enregistrement.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 132, "description": 2539, "whatsNew": 397, "keywordBytes": 78 }
}
```
...
```json
{
  ...
  "whatsNew": "Gli avvisi di rilevamento vocale ora richiedono 10 secondi di voce rilevata prima di avvisarti. Se consentito, usano notifiche urgenti, così è meno probabile che i riepiloghi delle notifiche o una full immersion ritardino un avviso immediato. I nuovi controlli mostrano l’accesso alle notifiche di macOS e un clic sull’avviso avvia la registrazione.",
  ...
  "counts": { "name": 5, "subtitle": 30, "promotionalText": 128, "description": 2477, "whatsNew": 349, "keywordBytes": 72 }
}
```
...
```json
{
  ...
  "whatsNew": "音声検出通知は、音声を10秒間検出してから通知するようになりました。許可されている場合は即時通知を使用するため、通知の要約や集中モードによってその場で必要な通知が遅れにくくなります。新しいコントロールでmacOSの通知アクセスを確認でき、通知をクリックすると録音が始まります。",
  ...
  "counts": { "name": 5, "subtitle": 9, "promotionalText": 55, "description": 1097, "whatsNew": 138, "keywordBytes": 89 }
}
```
...
```json
{
  ...
  "whatsNew": "음성 감지 알림은 이제 음성이 10초 동안 감지된 후에만 표시됩니다. 허용된 경우 긴급한 알림을 사용하므로 알림 요약이나 집중 모드로 인해 즉시 필요한 알림이 지연될 가능성이 줄어듭니다. 새 제어 항목에서 macOS 알림 접근 권한을 확인할 수 있으며, 알림을 클릭하면 녹음이 시작됩니다.",
  ...
  "counts": { "name": 5, "subtitle": 12, "promotionalText": 66, "description": 1265, "whatsNew": 161, "keywordBytes": 92 }
}
```
...
```json
{
  ...
  "whatsNew": "Thông báo Phát hiện Giọng nói giờ chỉ xuất hiện sau khi phát hiện giọng nói trong 10 giây. Khi được cho phép, cảnh báo giọng nói dùng thông báo gấp để lời nhắc đúng lúc ít bị trì hoãn bởi bản tóm tắt thông báo hoặc chế độ Tập trung. Các điều khiển mới hiển thị quyền truy cập thông báo của macOS và thao tác bấm vào cảnh báo sẽ bắt đầu ghi.",
  ...
  "counts": { "name": 5, "subtitle": 22, "promotionalText": 106, "description": 2146, "whatsNew": 340, "keywordBytes": 70 }
}
```
...
```json
{
  ...
  "whatsNew": "语音检测提醒现在会在检测到 10 秒语音后才通知你。获得允许后，语音提醒会使用即时通知，降低当下提醒被通知摘要或专注模式延迟的可能性。新的控制项会显示 macOS 通知权限，点按提醒即可开始录音。",
  ...
  "counts": { "name": 5, "subtitle": 7, "promotionalText": 53, "description": 822, "whatsNew": 98, "keywordBytes": 86 }
}
```
...
```json
{
  ...
  "whatsNew": "Os Alertas de Detecção de Voz agora exigem 10 segundos de fala detectada antes de avisar. Quando permitido, usam alertas relevantes para reduzir a chance de um aviso oportuno ser atrasado por resumos de notificações ou pelo Foco. Novos controles mostram o acesso às notificações do macOS, e clicar em um alerta inicia a gravação.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 122, "description": 2280, "whatsNew": 329, "keywordBytes": 73 }
}
```
...
```json
{
  ...
  "whatsNew": "Os Alertas de Deteção de Voz agora exigem 10 segundos de fala detetada antes de avisar. Quando permitido, usam avisos urgentes para reduzir a probabilidade de um aviso oportuno ser atrasado por resumos de notificações ou pela Concentração. Novos controlos mostram o acesso às notificações do macOS e clicar num aviso inicia a gravação.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 110, "description": 2352, "whatsNew": 335, "keywordBytes": 76 }
}
```

## Shared App Review notes

App Review notes are not a localized storefront field. Use the single English value in `docs/app-review/review-notes.txt`, which stays below Apple's 4,000-byte limit and explains the 1.2.2 system-aware notification controls, Time Sensitive entitlement and regenerated profile, local-only delivery, guarded recording action, sustained-speech gate, and no-account path.
