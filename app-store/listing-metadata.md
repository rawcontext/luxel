...
This file is the checked-in source of truth for Luxel's Mac App Store product-page metadata. It records the verified public listing as of August 22, 2026, the localized text metadata for version 1.5.0, and the localized screenshot gallery derived from the six English App Store masters in `app-store/screenshots`. Use the guarded App Store workflows to synchronize the editable App Store Connect version; do not edit the draft independently.

## Publication state

- TestFlight workflow run [32657756015](https://github.com/rawcontext/luxel/actions/runs/32657756015) uploaded macOS version 1.2.0, build `202608231822`, from release tag `v1.2.0`.
...
- The guarded App Store Metadata Draft workflow exports all 11 localized listings and the exact App Review notes while explicitly skipping binaries, screenshots, review submission, and release. [Run 32658321002](https://github.com/rawcontext/luxel/actions/runs/32658321002) synchronized the version 1.2.0 metadata draft successfully.
- [Run 32746887986](https://github.com/rawcontext/luxel/actions/runs/32746887986) synchronized the localized What's New text and App Review notes for version 1.2.2 without changing the binary, screenshots, review submission, or release settings.
- [Run 32782940879](https://github.com/rawcontext/luxel/actions/runs/32782940879) built, signed, and uploaded macOS version 1.3.0, build `202608242206`, to App Store Connect and TestFlight from release tag `v1.3.0`.
- [Run 32783943577](https://github.com/rawcontext/luxel/actions/runs/32783943577) synchronized the concise localized Copy as Markdown What's New text and App Review notes for version 1.3.0.
- [Run 32784038369](https://github.com/rawcontext/luxel/actions/runs/32784038369) selected build `202608242206`, submitted version 1.3.0 for App Review, and enabled automatic release after approval without a phased rollout.
- The App Store Localized Screenshots workflow renders 66 upload assets from the six real English masters, validates Apple's image requirements, and synchronizes them without uploading a binary, changing text metadata, submitting for review, or releasing the version. [Run 32660368255](https://github.com/rawcontext/luxel/actions/runs/32660368255) replaced the six prior English screenshots and uploaded all 66 committed assets to the version 1.2.0 draft successfully.
...
```json
{
  ...
  "whatsNew": "Automatic titles for new recordings. New recordings are organized into folders with their transcripts and exports.",
  ...
  "counts": { "name": 5, "subtitle": 24, "promotionalText": 134, "description": 2338, "whatsNew": 114, "keywordBytes": 88 }
}
```
...
```json
{
  ...
  "whatsNew": "Automatische Titel für neue Aufnahmen. Neue Aufnahmen werden mit ihren Transkripten und Exporten in Ordnern organisiert.",
  ...
  "counts": { "name": 5, "subtitle": 27, "promotionalText": 136, "description": 2476, "whatsNew": 120, "keywordBytes": 74 }
}
```
...
```json
{
  ...
  "whatsNew": "Títulos automáticos para nuevas grabaciones. Las nuevas grabaciones se organizan en carpetas con sus transcripciones y exportaciones.",
  ...
  "counts": { "name": 5, "subtitle": 30, "promotionalText": 121, "description": 2390, "whatsNew": 133, "keywordBytes": 77 }
}
```
...
```json
{
  ...
  "whatsNew": "Titres automatiques pour les nouveaux enregistrements. Les nouveaux enregistrements sont organisés en dossiers avec leurs transcriptions et fichiers exportés.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 132, "description": 2539, "whatsNew": 158, "keywordBytes": 78 }
}
```
...
```json
{
  ...
  "whatsNew": "Titoli automatici per le nuove registrazioni. Le nuove registrazioni sono organizzate in cartelle con trascrizioni e file esportati.",
  ...
  "counts": { "name": 5, "subtitle": 30, "promotionalText": 128, "description": 2477, "whatsNew": 132, "keywordBytes": 72 }
}
```
...
```json
{
  ...
  "whatsNew": "新しい録画・録音のタイトルを自動生成。新しい録画・録音を、文字起こしや書き出したファイルと一緒にフォルダで整理します。",
  ...
  "counts": { "name": 5, "subtitle": 9, "promotionalText": 55, "description": 1097, "whatsNew": 59, "keywordBytes": 89 }
}
```
...
```json
{
  ...
  "whatsNew": "새 녹화와 녹음에 제목을 자동으로 생성합니다. 새 녹화와 녹음을 대본 및 내보낸 파일과 함께 폴더로 정리합니다.",
  ...
  "counts": { "name": 5, "subtitle": 12, "promotionalText": 66, "description": 1265, "whatsNew": 62, "keywordBytes": 92 }
}
```
...
```json
{
  ...
  "whatsNew": "Tự động tạo tiêu đề cho bản ghi mới. Các bản ghi mới được sắp xếp vào thư mục cùng bản chép lời và tệp xuất.",
  ...
  "counts": { "name": 5, "subtitle": 22, "promotionalText": 106, "description": 2146, "whatsNew": 108, "keywordBytes": 70 }
}
```
...
```json
{
  ...
  "whatsNew": "为新录制自动生成标题。新录制会与转录文本和导出文件一起按文件夹整理。",
  ...
  "counts": { "name": 5, "subtitle": 7, "promotionalText": 53, "description": 822, "whatsNew": 34, "keywordBytes": 86 }
}
```
...
```json
{
  ...
  "whatsNew": "Títulos automáticos para novas gravações. As novas gravações são organizadas em pastas com suas transcrições e exportações.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 122, "description": 2280, "whatsNew": 123, "keywordBytes": 73 }
}
```
...
```json
{
  ...
  "whatsNew": "Títulos automáticos para novas gravações. As novas gravações são organizadas em pastas com as respetivas transcrições e exportações.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 110, "description": 2352, "whatsNew": 132, "keywordBytes": 76 }
}
```

## Shared App Review notes

App Review notes are not a localized storefront field. Use the single English value in `app-store/review-notes.txt`, which stays below Apple's 4,000-byte limit and explains how to review version 1.5.0.
