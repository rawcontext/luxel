...
```json
{
  ...
  "whatsNew": "Show or hide timestamps in Markdown transcripts directly from the editor. Also includes bug fixes and performance improvements.",
  ...
  "counts": { "name": 5, "subtitle": 24, "promotionalText": 134, "description": 2338, "whatsNew": 127, "keywordBytes": 88 }
}
```
...
```json
{
  ...
  "whatsNew": "Zeitstempel in Markdown-Transkripten direkt im Editor ein- oder ausblenden. Außerdem enthalten: Fehlerbehebungen und Leistungsverbesserungen.",
  ...
  "counts": { "name": 5, "subtitle": 27, "promotionalText": 136, "description": 2476, "whatsNew": 141, "keywordBytes": 74 }
}
```
...
```json
{
  ...
  "whatsNew": "Muestra u oculta las marcas de tiempo en las transcripciones en Markdown directamente desde el editor. También incluye correcciones de errores y mejoras de rendimiento.",
  ...
  "counts": { "name": 5, "subtitle": 30, "promotionalText": 121, "description": 2390, "whatsNew": 168, "keywordBytes": 77 }
}
```
...
```json
{
  ...
  "whatsNew": "Affichez ou masquez les horodatages dans les transcriptions Markdown directement depuis l’éditeur. Inclut également des corrections de bugs et des améliorations des performances.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 132, "description": 2539, "whatsNew": 178, "keywordBytes": 78 }
}
```
...
```json
{
  ...
  "whatsNew": "Mostra o nascondi i timestamp nelle trascrizioni Markdown direttamente dall’editor. Include anche correzioni di bug e miglioramenti delle prestazioni.",
  ...
  "counts": { "name": 5, "subtitle": 30, "promotionalText": 128, "description": 2477, "whatsNew": 150, "keywordBytes": 72 }
}
```
...
```json
{
  ...
  "whatsNew": "エディタから直接、Markdown形式の文字起こしのタイムスタンプを表示・非表示に切り替えられるようになりました。不具合の修正とパフォーマンスの改善も含まれています。",
  ...
  "counts": { "name": 5, "subtitle": 9, "promotionalText": 55, "description": 1097, "whatsNew": 83, "keywordBytes": 89 }
}
```
...
```json
{
  ...
  "whatsNew": "편집기에서 Markdown 전사문의 타임스탬프를 표시하거나 숨길 수 있습니다. 버그 수정 및 성능 개선도 포함되어 있습니다.",
  ...
  "counts": { "name": 5, "subtitle": 12, "promotionalText": 66, "description": 1265, "whatsNew": 69, "keywordBytes": 92 }
}
```
...
```json
{
  ...
  "whatsNew": "Hiện hoặc ẩn dấu thời gian trong bản chép lời Markdown ngay trong trình chỉnh sửa. Bản cập nhật cũng bao gồm các bản sửa lỗi và cải thiện hiệu năng.",
  ...
  "counts": { "name": 5, "subtitle": 22, "promotionalText": 106, "description": 2146, "whatsNew": 148, "keywordBytes": 70 }
}
```
...
```json
{
  ...
  "whatsNew": "可直接在编辑器中显示或隐藏 Markdown 转录文本中的时间戳。还包括错误修复和性能改进。",
  ...
  "counts": { "name": 5, "subtitle": 7, "promotionalText": 53, "description": 822, "whatsNew": 46, "keywordBytes": 86 }
}
```
...
```json
{
  ...
  "whatsNew": "Mostre ou oculte os marcadores de tempo nas transcrições em Markdown diretamente no editor. Também inclui correções de bugs e melhorias de desempenho.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 122, "description": 2280, "whatsNew": 150, "keywordBytes": 73 }
}
```
...
```json
{
  ...
  "whatsNew": "Mostre ou oculte as marcas de tempo nas transcrições em Markdown diretamente no editor. Inclui também correções de erros e melhorias de desempenho.",
  ...
  "counts": { "name": 5, "subtitle": 25, "promotionalText": 110, "description": 2352, "whatsNew": 147, "keywordBytes": 76 }
}
```

## Shared App Review notes

App Review notes are not a localized storefront field. Use the single English value in `app-store/review-notes.txt`, which stays below Apple's 4,000-byte limit and explains how to review version 1.4.0.
