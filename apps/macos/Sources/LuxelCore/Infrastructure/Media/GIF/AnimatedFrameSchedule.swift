...
struct AnimatedFrameSchedule: Equatable, Sendable {
    ...
    init(request: ExportRequest, sourceFrameRate: Double) {
        ...
        let candidateFrameCount = max(1, Int((request.timeRange.duration * requestedFrameRate).rounded()))
...
func animatedFrameSchedule(for request: ExportRequest, asset: AVURLAsset) async -> AnimatedFrameSchedule {
    let sourceFrameRate = await sourceFrameRate(for: asset, fallback: request.frameRate.framesPerSecond)
