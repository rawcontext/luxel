    ...
    public static func viewModel(for activity: NotchActivity) -> NotchActivityViewModel {
        ...
        case .recording(let elapsed, let audioLevel, let muted):
            ...
            return NotchActivityViewModel(
                ...
                trailingEarText: muted ? "Muted" : "Mic \(levelPercent(audioLevel.peak))",
                audioLevel: muted ? nil : audioLevel,
                actions: [
                    ...
                    action(.toggleMute, muted ? "Unmute" : "Mute", muted ? "mic" : "mic.slash")
                ],
        ...
        case .error(let error):
            var actions: [NotchActivityActionDescriptor] = []
            if let recoveryAction = error.recoveryAction {
                actions.append(actionDescriptor(for: recoveryAction))
            }
